...
## Immediate
- Add an explicit queue-quality pass before manual apply prep and tailored resume work.
...
- Default queue order to the highest-value pending job first for manual application work.
...
- Add a preflight command that lists “eligible”, “manual review”, and “closed/stale” buckets before manual submission.
- Keep `manual-review-needed` jobs in `pending`, but make that state visible in the manual prep workflow.
...
- If prep is unavailable, surface the issue before manual submission.
...
## Dashboard
- Show a dedicated queue-quality panel on the apply receipts or manual prep page.
