...
const _profileDir = require('path').resolve(process.env.JOB_PROFILE_DIR || require('path').join(__dirname, '..', '..', 'profiles', 'example'));
const autoApplyConfig = require(_profileDir + '/auto-apply-config');
...
const AUTO_APPLY_SUPPORTED = new Set(['greenhouse', 'lever', 'ashby']);
...
function detectAutoApplyPlatform(job) {
  const direct = detectAts(job.url || '');
  if (direct?.platform) return direct.platform.toLowerCase();
...
function getAutoApplyBadge(job) {
  if (job.auto_apply_status === 'success') {
    return {
      className: 'auto-applied',
      label: 'auto',
      title: `Auto-applied on ${job.auto_applied_at ? job.auto_applied_at.slice(0, 10) : '?'}`,
    };
  }

  if (job.auto_apply_status === 'failed') {
    return {
      className: 'auto-failed',
      label: 'auto&#10007;',
      title: `Auto-apply failed: ${job.auto_apply_error || ''}`,
    };
  }
  ...
  if (job.apply_complexity === 'complex') {
    return {
      className: 'auto-failed',
      label: 'auto&#10007;',
      title: 'Auto-apply blocked: complex application form',
    };
  }

  if (job.apply_complexity !== 'simple') {
    return {
      className: 'auto-failed',
      label: 'auto&#10007;',
      title: 'Auto-apply blocked: complexity not classified as simple',
    };
  }

  const platform = detectAutoApplyPlatform(job);
  if (!platform || !AUTO_APPLY_SUPPORTED.has(platform)) {
    return {
      className: 'auto-failed',
      label: 'auto&#10007;',
      title: 'Auto-apply blocked: unsupported platform',
    };
  }

  if ((autoApplyConfig.platformBlocklist || []).includes(platform)) {
    return {
      className: 'auto-failed',
      label: 'auto&#10007;',
      title: `Auto-apply blocked: ${platform} is disabled in unattended mode`,
    };
  }

  return {
    className: 'auto-ready',
    label: 'auto',
    title: `Eligible for unattended apply via ${platform}`,
  };
}
...
function renderJobListHeader(filter, sort, level, searchOptions) {
  ...
  <button class="sort-col-btn${dateActive ? ' active' : ''}" onclick="location='${buildDashboardHref(filter, 'date', level, searchOptions)}'">Date ${dateActive ? '&#8595;' : '&#8593;'}</button>
...
function renderJobTable(jobs, appliedByCompany, companyTags, filter, sort, level, pagination, searchOptions = {}) {
  ...
  const cards = jobs.map(j => {
    ...
    if (j.apply_complexity === 'simple') {
      badgeParts.push('<span class="complexity-badge simple" title="Quick apply">simple</span>');
    } else if (j.apply_complexity === 'complex') {
      badgeParts.push('<span class="complexity-badge complex" title="Multi-step form">complex</span>');
    }
    ...
    const canAutoApply = j.apply_complexity === 'simple'
      && j.auto_apply_status !== 'success'
      && j.status === 'pending';
    ...
    const colDate = fmtColDate(j.posted_at);
    ...
    <div class="job-card${cardExtra}" data-id="${eid}" data-status="${escapeHtml(j.status)}" data-posted-ts="${postedTimestamp(j.posted_at)}" data-applied-ts="${postedTimestamp(j.applied_at)}" style="--score-color:${scoreColor(sc)}">
      ...
      <div class="job-col-actions">
        ...
        <div class="job-actions-menu" id="jmenu-${eid}">
          ...
          <a class="job-action-item" href="/job-application-prep?id=${eid}" target="_blank" rel="noreferrer" onclick="closeJobMenu('${eid}')">${ICON_FILE} Application Prep</a>
          ...
          ${canAutoApply ? `<button class="job-action-item" id="auto-apply-btn-${eid}" onclick="autoApplyJob('${eid}', this); closeJobMenu('${eid}')">${ICON_SEND} Apply Now</button>` : ''}
  ...
  return renderJobListHeader(filter, sort, level, searchOptions)
    + `<div class="job-list" id="job-tbody">${cards}</div>`
