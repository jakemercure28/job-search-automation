...
function renderFilters(filter, sort, globalStats, level, searchOptions = {}) {
  ...
  const secFilters = [
    ...
    return `<a href="${buildDashboardHref(id, sort, level, searchOptions)}" class="menu-item${active ? ' active' : ''}">${label}${count ? ` <span class="menu-count">${count}</span>` : ''}</a>`;
  }).join('');
...
<div class="header-sub">
  <div class="dd-wrap">
    ...
    <div id="nav-menu" class="nav-dropdown">
      ...
      <a href="${buildDashboardHref('auto-apply-log', sort, null, searchOptions)}" class="menu-item${filter === 'auto-apply-log' ? ' active' : ''}">Auto-Apply Log</a>
