...
function renderCommandBullets(commands) {
  return commands.map((item) => {
    const flags = item.flags.length
      ? item.flags.map((flag) => `<code>${escapeHtml(flag)}</code>`).join(', ')
      : 'none';

    return [
      `<code>${escapeHtml(item.command)}</code>: ${escapeHtml(item.description)}`,
      `Use when: ${escapeHtml(item.when)}`,
      `Flags: ${flags}`,
    ].join('<br>');
  });
}

function renderNpmCommandSection() {
  const commandGroups = getCommandGroups();

  return `
  <section class="help-page-section">
    <div class="help-page-section-head">
      <div class="help-page-kicker">NPM Commands</div>
      <h2>Which Command To Run</h2>
    </div>
    <div class="help-page-grid help-page-grid-2">
      ${commandGroups.map((group) => renderCard({
        eyebrow: 'Command group',
        title: group.title,
        body: escapeHtml(group.description),
        bullets: renderCommandBullets(group.commands),
      })).join('')}
    </div>
  </section>
  `;
...
function renderHelpPage() {
  ...
  const routesCode = [
    'GET  /                     dashboard page',
    'GET  /help                 architecture + system docs + npm commands',
    ...
    <div class="help-page-grid help-page-grid-2">
      ...
      ${renderCard({
        ...
        bullets: [
          ...
          'After the profile loop, the repo also runs <code>node scripts/validate-slugs.js --broken-only</code> and <code>node scripts/update-context.js</code>',
        ],
      })}
      ${renderCard({
        eyebrow: 'Scraping',
