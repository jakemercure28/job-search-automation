...
function renderAutoApplies(data) {
  if (!data) return '<div class="empty">No auto-apply data available.</div>';
...
<div class="analytics-wrap">
  <div class="analytics-section market-section">
    <h2 class="analytics-title">Auto Applies</h2>
    <p class="analytics-hint"><span style="font-family:var(--font-mono)">${summary.total}</span> attempts in view. Receipts include the PDF used, screenshots, actor, and failure class.</p>
    ...
    ${renderPaginatedTable(
      'auto-applies-table',
      10,
      `<thead><tr><th>Time</th><th>Company</th><th>Role</th><th>Score</th><th>Platform</th><th>Mode</th><th>PDF</th><th>Artifacts</th><th>Status</th><th>Error</th></tr></thead><tbody>${attemptRows || '<tr><td colspan="10" style="text-align:center;color:var(--text-muted)">No auto-apply attempts match these filters</td></tr>'}</tbody>`,
      rows.length,
      '&larr; Newer',
      'Older &rarr;',
      'pageAutoApplies'
