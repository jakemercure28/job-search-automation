...
const { parseYearsFromDescription, classifySeniority, levelFromYears } = require('../seniority');
...
function renderSeniorityBreakdown(allJobs, applicantYoe) {
  ...
  const applicantLevel = (applicantYoe != null && !isNaN(applicantYoe)) ? levelFromYears(applicantYoe) : 'mid';
  ...
  const yoe = (applicantYoe != null && !isNaN(applicantYoe)) ? applicantYoe : 4;
  const accessible = allJobs.filter(j => {
    const { level, years } = classifySeniority(j.title, j.description);
    if (years !== null) return years <= yoe;
    ...
    <div class="analytics-section market-section">
      ...
      <div style="margin-top:20px;padding:14px 18px;background:rgba(255,255,255,0.04);border-radius:8px;border-left:3px solid #a855f7">
        <div style="font-size:14px;color:#e2e8f0">
          <strong style="font-family:var(--font-mono)">${accessible.length}</strong> of <span style="font-family:var(--font-mono)">${total}</span> roles (<strong style="font-family:var(--font-mono)">${accessiblePct}%</strong>) are realistically accessible at ${yoe} YOE
          <span style="font-size:12px;color:${COLORS.muted}"> \u2014 roles asking \u2264${yoe} years or titled mid-level/junior</span>
        </div>
      </div>
    </div>
    ${experienceCard}
...
<div class="analytics-section market-section tracker-section">
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
    <h2 class="analytics-title" style="margin-bottom:0">Pipeline Over Time</h2>
    <div class="tracker-period-btns">
      <button class="tracker-period-btn" data-period="7d">7D</button>
      <button class="tracker-period-btn active" data-period="30d">30D</button>
      <button class="tracker-period-btn" data-period="90d">90D</button>
      <button class="tracker-period-btn" data-period="all">All</button>
    </div>
  </div>
  <p class="analytics-hint" style="margin-bottom:16px">Active jobs (pending, applied, interviewing) tracked over time. Snapshots are taken every 30 minutes when the server is running.</p>
  <div class="tracker-chart-wrap">
    <canvas id="trackerChart"></canvas>
    <div id="trackerEmpty" class="tracker-empty" style="display:none">
      No snapshot data yet. Data will accumulate automatically while the server is running.
    </div>
  </div>
</div>
<script>
(function() {
  ...
  function decimate(rows, target) {
    ...
    var step = Math.ceil(rows.length / target);
    var out = [];
    for (var i = 0; i < rows.length; i += step) {
      var slice = rows.slice(i, i + step);
      var mid = slice[Math.floor(slice.length / 2)];
      out.push({
        recorded_at: mid.recorded_at,
        pending: Math.round(slice.reduce(function(s,r){return s+r.pending;},0) / slice.length),
        applied: Math.round(slice.reduce(function(s,r){return s+r.applied;},0) / slice.length),
        interviewing: Math.round(slice.reduce(function(s,r){return s+r.interviewing;},0) / slice.length),
      });
    }
    return out;
  }
  ...
  function loadTracker(period) {
    fetch('/api/tracker?period=' + period)
      .then(function(r){return r.json();})
      .then(function(rows) {
        var empty = document.getElementById('trackerEmpty');
        var canvas = document.getElementById('trackerChart');
        ...
        var pts = decimate(rows, 120);
        var labels = pts.map(function(r){return formatLabel(r.recorded_at, period);});
        var datasets = [
          {
            label: 'Pending',
            data: pts.map(function(r){return r.pending;}),
            borderColor: '#f59e0b',
          ...
          {
            label: 'Applied',
            data: pts.map(function(r){return r.applied;}),
            borderColor: '#60a5fa',
          ...
          {
            label: 'Interviewing',
            data: pts.map(function(r){return r.interviewing;}),
            borderColor: '#34d399',
        ...
        if (trackerChart) {
          ...
          trackerChart.data.datasets = datasets;
          trackerChart.update();
        } else {
  ...
  document.querySelectorAll('.tracker-period-btn').forEach(function(btn) {
    btn.addEventListener('click', function() {
      document.querySelectorAll('.tracker-period-btn').forEach(function(b){b.classList.remove('active');});
      btn.classList.add('active');
      currentPeriod = btn.dataset.period;
      loadTracker(currentPeriod);
    });
  });
...
function renderMarketResearch(cache, jobCount, allJobs, applicantYoe) {
  const trackerHtml = renderTrackerSection();
  const seniorityHtml = renderSeniorityBreakdown(allJobs, applicantYoe);

  if (!cache) return trackerHtml + seniorityHtml + renderRunPage(null, jobCount);
  const stale = Date.now() - cache.generatedAt > 48 * 3600 * 1000;
  if (stale) return trackerHtml + seniorityHtml + renderRunPage(cache.generatedAt, jobCount);
  return trackerHtml + seniorityHtml + renderResults(cache);
}
