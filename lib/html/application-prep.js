...
const { DASHBOARD_BASE_URL } = require('../../config/constants');
...
function workflowLabel(workflow) {
  if (workflow === 'simple-auto') return 'simple manual';
...
function renderQuestionList(prep) {
  ...
  if (prep.status === 'unsupported') {
    return `<section class="prep-empty-state">
      <h2>Fast-failed</h2>
      <p>${escapeHtml(prep.summary || prep.error || 'This form needs manual review.')}</p>
    </section>`;
  }
  ...
  if (!Array.isArray(prep.questions) || prep.questions.length === 0) {
    return `<section class="prep-empty-state">
      ...
      <p>${escapeHtml(prep.summary || 'No custom answers were detected. Open the apply URL and submit manually.')}</p>
    </section>`;
  }
...
window.__prepBookmarklet = ${JSON.stringify(`javascript:(function(){var s=document.createElement('script');s.src='${DASHBOARD_BASE_URL}/job-bookmarklet.js?id=${encodeURIComponent(job.id)}&t='+Date.now();document.documentElement.appendChild(s);}())`)};
