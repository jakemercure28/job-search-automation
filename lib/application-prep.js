...
function looksLikeAuthorizationQuestion(label) {
  return /legally authorized|authorized to lawfully work|authorized to work|authorized to reside and work|reside and work in the country/.test(label);
}
...
function heuristicAnswer(field, sources) {
  ...
  if (/eligible to work in the country/.test(label)) return chooseOption(options, ['Yes']) || 'Yes';
  ...
  if (/country of residence|current country of employment|country of employment|citizenship/.test(label)) {
    return chooseOption(options, ['United States of America', 'United States']) || 'United States';
  }
  ...
  if (/state of residence|state in which you currently reside/.test(label)) return chooseOption(options, ['Washington', 'WA']) || 'Washington';
  if (/zip code|zip \/ postal|postal code/.test(label)) return applicant.zipCode || null;
  ...
  if (/gender/.test(label)) return chooseOption(options, [applicant.eeoGender, 'Decline to self-identify', 'Decline to answer']) || applicant.eeoGender;
  if (/race|ethnicity/.test(label)) return chooseOption(options, [applicant.eeoRace, 'Decline to self-identify', 'Decline to answer']) || applicant.eeoRace;
  if (/veteran/.test(label)) return chooseOption(options, [applicant.eeoVeteran, 'I am not a veteran', 'Decline to self-identify', 'Decline to answer']) || applicant.eeoVeteran;
  if (/disability/.test(label)) return chooseOption(options, [applicant.eeoDisability, "Don't wish to answer", 'Decline to self-identify', 'Decline to answer']) || applicant.eeoDisability;
...
async function runApplyExtract(jobId) {
  ...
  const result = parseJsonLoose(stdout);
