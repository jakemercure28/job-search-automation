...
function fetchFilteredJobs(db, filter, sort, level) {
  ...
  const filterQueries = {
    ...
    'not-applied':  () => db.prepare(`SELECT * FROM jobs WHERE status NOT IN ('applied','responded','archived','closed') AND COALESCE(stage, '') != 'closed' ORDER BY ${orderBy}`).all(),
...
const handleAutoApply = postRoute(async ({ id }, res, db) => {
  ...
  const result = await applyOne(db, autoApplyConfig, id, false, { actor: 'manual' });
  if (result.success) {
    jsonOk(res, {
      ok: true,
