...
function fetchFilteredJobs(db, filter, sort, level) {
  ...
  const filterQueries = {
    ...
    'rejected':     () => db.prepare(`SELECT * FROM jobs WHERE stage = 'rejected' ORDER BY COALESCE(rejected_at, updated_at) DESC, updated_at DESC`).all(),
...
function handleDashboardPage(req, res, db, url) {
  ...
  const requestedSort = (url.searchParams.get('sort') || 'score') === 'date' ? 'date' : 'score';
  const sort = filter === 'rejected' ? 'date' : requestedSort;
