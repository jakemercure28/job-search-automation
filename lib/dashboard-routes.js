...
const { applyOne } = require('./auto-applier');
...
function fetchAnalyticsData(db) {
  ...
  return { allTimeStats, funnel, scoreCalibration, recentEvents, rejectionInsights, autoApplyLog };
}
...
const handleAutoApply = postRoute(async ({ id }, res, db) => {
  if (!id) { jsonError(res, 400, 'id required'); return; }
  ...
  const result = await applyOne(db, autoApplyConfig, id);
  if (result.success) {
    jsonOk(res, { ok: true, securityCode: result.securityCode || null });
  } else {
    jsonError(res, 422, result.error || 'apply failed');
  }
});
