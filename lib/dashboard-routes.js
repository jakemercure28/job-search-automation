...
const { analyzeRejection, scoreRejectionLikelihood, callGemini } = require('../scorer');
...
const { toLocalDateString, getScraperHealth, getTodayActivityCounts, getDailyManualApplyCounts, buildDailyDigest } = require('./dashboard-insights');
const { isAccessible } = require('./seniority');
...
const handleRejectionAnalysis = route((req, res, db, url) => {
  const id = url.searchParams.get('id');
  const job = db.prepare('SELECT rejection_analysis, transcript_path, rejected_from_stage, rejected_at FROM jobs WHERE id=?').get(id);
  if (!job) { jsonError(res, 404, 'job not found'); return; }
  jsonOk(res, {
    analysis: job.rejection_analysis,
    transcriptPath: job.transcript_path,
    rejectedFromStage: job.rejected_from_stage,
...
const ORDER_BY = {
  ...
  'score-applied': 'COALESCE(score, claude_score) DESC, applied_at DESC, created_at DESC',
  'score-posted':  'COALESCE(score, claude_score) DESC, posted_at DESC, created_at DESC',
};
const JOBS_PAGE_SIZE = 25;

function fetchFilteredJobs(db, filter, sort, level) {
  const dateKey = filter === 'applied' ? 'applied' : 'posted';
  const orderBy = ORDER_BY[`${sort === 'date' ? 'date' : 'score'}-${dateKey}`];

  const filterQueries = {
    'all':          () => db.prepare(`SELECT * FROM jobs WHERE status != 'archived' ORDER BY ${orderBy}`).all(),
    'not-applied':  () => db.prepare(`SELECT * FROM jobs WHERE status NOT IN ('applied','responded','archived') ORDER BY ${orderBy}`).all(),
    'applied':      () => db.prepare(`SELECT * FROM jobs WHERE status IN ('applied','responded') ORDER BY ${orderBy}`).all(),
    'interviewing': () => db.prepare(`SELECT * FROM jobs WHERE stage IN ('phone_screen','interview','onsite','offer') ORDER BY ${orderBy}`).all(),
    'rejected':     () => db.prepare(`SELECT * FROM jobs WHERE stage = 'rejected' ORDER BY rejected_at DESC, updated_at DESC`).all(),
...
function fetchAnalyticsData(db) {
  ...
  const scoreCalibration = db.prepare(`
    SELECT COALESCE(score, claude_score) as score,
      COUNT(*) as total,
    ...
    FROM jobs WHERE COALESCE(score, claude_score) IS NOT NULL
    GROUP BY COALESCE(score, claude_score) ORDER BY score
  `).all();
  ...
  const rejectionInsights = db.prepare(`
    SELECT e.from_value as rejected_from,
      j.company, j.title, COALESCE(j.score, j.claude_score) as score, j.posted_at, j.applied_at,
  ...
  const scoreComparison = db.prepare(`
    SELECT id, title, company, location, score AS gemini_score, claude_score,
           reasoning AS gemini_reasoning, claude_reasoning,
           ABS(score - claude_score) AS diff
    FROM jobs
    WHERE claude_score IS NOT NULL
    ORDER BY diff DESC
  `).all();

  const autoApplyLog = db.prepare(`
    SELECT l.job_id, l.attempted_at, l.status, l.error, l.resume_filename, l.security_code, l.dry_run,
      j.company, j.title, j.score
  ...
  return { allTimeStats, funnel, scoreCalibration, recentEvents, rejectionInsights, scoreComparison, autoApplyLog };
}
...
const MARKET_RESEARCH_CACHE_PATH = path.join(
  process.env.JOB_PROFILE_DIR || path.join(__dirname, '..', 'profiles', 'example'),
  'market-research-cache.json'
);
const RESUME_PATH_FOR_RESEARCH = path.join(
  process.env.JOB_PROFILE_DIR || path.join(__dirname, '..', 'profiles', 'example'),
  'resume-ai.md'
);
...
function handleDashboardPage(req, res, db, url) {
  ...
  const sort = (url.searchParams.get('sort') || 'score') === 'date' ? 'date' : 'score';
  ...
  const isPaginatedListView = !['analytics', 'auto-apply-log', 'activity-log', 'market-research'].includes(filter);
  ...
  const analyticsData = ['analytics', 'auto-apply-log', 'activity-log'].includes(filter) ? fetchAnalyticsData(db) : null;
