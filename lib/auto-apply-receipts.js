...
function classifyAutoApplyFailure(error, result = {}) {
  ...
  if (!text) return null;
  if (/manual review required|unresolved fields/.test(text)) return 'manual-review-needed';
  if (/job is no longer available|redirected away from job|application form not detected|error page for this job/.test(text)) return 'closed-page';
  if (/rate limited|rate-limited|503|provider throttled|gemini/.test(text)) return 'provider-throttled';
...
function recordAutoApplyAttempt(db, {
  ...
  const detailsJson = result?.details ? JSON.stringify(result.details) : null;
  ...
  const insert = db.prepare(`
    INSERT INTO auto_apply_log (
      ...
      resume_path, prep_generated_at, actor, display_error, details_json
    )
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const logLevel = status === 'success' ? 'info' : status === 'failed' ? 'warn' : 'info';
  log[logLevel]('Attempt recorded', {
    jobId: job.id,
    company: job.company,
    status,
...
function insertAutoApplyRun(db, {
  ...
  log.info('Run started', { runId: id, mode, actor, dryRun });
  return id;
}
...
function finishAutoApplyRun(db, runId, summary = {}, completedAt = nowIso()) {
  ...
  log.info('Run finished', { runId, ...summary });
}
...
function listAutoApplyAttempts(db, {
  ...
  if (failureClass) rows = rows.filter((row) => String(row.failure_class || '').toLowerCase() === String(failureClass).toLowerCase());
  if (mode) {
...
function summarizeAutoApplyAttempts(rows) {
  ...
  for (const row of rows) {
    ...
    if (row.status === 'success') summary.submitted += 1;
      ...
      summary.failed += 1;
      if (!['duplicate', 'abuse-warning', 'closed-page', 'manual-review-needed'].includes(row.failure_class)) {
        summary.retryNeeded += 1;
      }
    }
  }
