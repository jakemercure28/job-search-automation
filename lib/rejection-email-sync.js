...
const REJECTION_EMAIL_SYNC_LOG = logPaths.daily('rejection-sync');
const log = require('./logger')('rejection-email-sync', { logFile: REJECTION_EMAIL_SYNC_LOG });
const {
...
function logCandidateDecision(mailbox, message, match, status, reason) {
  log.info('Rejection email candidate processed', {
    mailbox,
    uid: message.uid,
    subject: message.subject || null,
    status,
    reason: reason || match?.reason || null,
    jobId: match?.job?.id || null,
    company: match?.job?.company || null,
    title: match?.job?.title || null,
...
async function sweepMailbox(db, mailbox, metadataPrefix, options) {
  ...
  const result = await fetchMessages({
    ...
    lastUid: options.replay ? null : state.lastUid,
    uidValidity: options.replay ? null : state.uidValidity,
  });

  const messages = typeof options.messageFilter === 'function'
    ? result.messages.filter(options.messageFilter)
    : result.messages;
  const summary = { fetched: messages.length, candidates: 0, applied: 0, dryRun: 0, ignored: 0, unmatched: 0 };
  ...
  for (const message of messages) {
    ...
    if (!match.job) {
      summary.unmatched += 1;
      if (!options.classifyOnly) logCandidateEmail(db, mailbox, result.uidValidity, message, match, 'unmatched', match.reason);
      logCandidateDecision(mailbox, message, match, 'unmatched', match.reason);
      continue;
    }

    const applied = options.classifyOnly
      ? { status: 'dry_run', reason: match.reason }
      : applyRejectionUpdate(db, message, match, { dryRun: options.dryRun });
    ...
    if (!options.classifyOnly) logCandidateEmail(db, mailbox, result.uidValidity, message, match, applied.status, applied.reason);
    logCandidateDecision(mailbox, message, match, applied.status, applied.reason);
  }

  if (!options.replay && !options.classifyOnly) {
    saveSyncMetadata(db, { lastUid: result.lastUid, uidValidity: result.uidValidity }, metadataPrefix);
  }
  log.info('Rejection mailbox sweep complete', { mailbox, ...summary });
  return summary;
}
...
function startRejectionEmailPoller(db, options = {}) {
  ...
  const runOnce = async () => {
    ...
    running = true;
    try {
      await syncRejectionEmails(db, options);
    } catch (error) {
