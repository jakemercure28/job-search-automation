...
function issueList(slugHealth, key) {
  return Array.isArray(slugHealth?.[key]) ? slugHealth[key] : [];
}
...
function formatSlugTimestamp(timestamp) {
  if (!timestamp) return '';
...
function renderSlugHealthBanner(slugHealth) {
  if (!slugHealth || slugHealth._dismissed) return '';

  const broken = issueList(slugHealth, 'broken');
  const blocked = issueList(slugHealth, 'blocked');
  const transient = issueList(slugHealth, 'transient');
  const issues = [
    { key: 'broken', items: broken, label: count => `${plural(count, 'broken ATS slug')}` },
    { key: 'blocked', items: blocked, label: count => `${plural(count, 'blocked check')}` },
    { key: 'transient', items: transient, label: count => `${plural(count, 'transient check')}` },
  ].filter(group => group.items.length);

  if (!issues.length) return '';

  const counts = issues.map(group => group.label(group.items.length)).join(', ');
  const affected = issues.flatMap(group => group.items.map(item => ({ ...item, category: group.key })));
  const preview = affected.slice(0, 6).map(item => {
    const slug = escapeHtml(item.slug || item.label || 'unknown');
    const ats = escapeHtml(item.ats || item.category);
    const status = item.status ? ` HTTP ${escapeHtml(String(item.status))}` : '';
    return `${ats}/${slug}${status}`;
  }).join(', ');
  const more = affected.length > 6 ? `, +${affected.length - 6} more` : '';
  const totalAttempts = affected.reduce((sum, item) => sum + (Number(item.attempts) || 1), 0);
  const attemptSummary = `${plural(totalAttempts, 'attempt')} across ${plural(affected.length, 'check')}`;
  const checked = formatSlugTimestamp(slugHealth.timestamp);
  const ts = checked ? `<span style="color:#888;margin-left:4px">Checked: ${checked}</span>` : '';
  ...
  <span style="flex:1"><strong>${counts}</strong>
  (${preview}${more}) — ${attemptSummary}; run <code style="background:#1a1a2e;padding:1px 4px;border-radius:3px">npm run validate-slugs:broken</code>
  ${ts}</span>
...
${renderSlugHealthBanner(slugHealth)}
${bodyHtml}
</div>
...
module.exports = { renderDashboard, renderSlugHealthBanner, COLORS };
