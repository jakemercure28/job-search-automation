...
const { renderAnalytics, renderAutoApplyLog, renderActivityLog } = require('./html/analytics');
...
function renderDashboard({ jobs, pagination, filter, sort, level, searchOptions, dailyDigest, dailyCounts, globalStats, appliedByCompany, apiUsage, scraperHealth, companyTags, analyticsData, marketResearchData, slugHealth, jdHealth }) {
  ...
  } else if (filter === 'auto-apply-log') {
    bodyHtml = renderAutoApplyLog(analyticsData);
  } else if (filter === 'activity-log') {
    bodyHtml = renderActivityLog(analyticsData);
  } else if (filter === 'market-research') {
    bodyHtml = renderMarketResearch(marketResearchData?.cache, marketResearchData?.jobCount || 0, marketResearchData?.allJobs);
  } else {
...
<link rel="icon" type="image/svg+xml" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'%3E%3Crect width='100' height='100' fill='%230a0f1a'/%3E%3Crect x='20' y='30' width='60' height='45' fill='%2322c55e' rx='3'/%3E%3Crect x='35' y='25' width='30' height='8' fill='%2322c55e' rx='2'/%3E%3Cpath d='M 40 25 Q 50 15 60 25' stroke='%2322c55e' stroke-width='2' fill='none'/%3E%3C/svg%3E">
...
${(() => {
  ...
  &mdash; run <code style="background:#1a1a2e;padding:1px 4px;border-radius:3px">node validate-slugs.js</code>
