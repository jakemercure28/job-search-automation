...
async function fillGreenhouseEducation(page, applicant) {
  ...
  let schoolInput = await page.$('input[id*="school"], input[placeholder*="school" i], input[name*="school" i]');
  if (!schoolInput) {
    const addBtn = await page.$('button[aria-label*="education" i], button[data-action*="education" i]');
    if (addBtn) {
      await addBtn.click();
      await new Promise((r) => setTimeout(r, 800));
      schoolInput = await page.$('input[id*="school"], input[placeholder*="school" i], input[name*="school" i]');
    }
  }
...
async function greenhouseFieldIsEmpty(page, fieldName) {
  return page.evaluate((targetFieldName) => {
    ...
    const hasValueContainer = group?.querySelector('[class*="value-container--has-value"]');
    const placeholder = group?.querySelector('[class*="placeholder"]');
    return !(
      (renderedValue?.innerText || '').trim()
      || (hasValueContainer && !(placeholder && (placeholder.innerText || '').trim()))
    );
  }, fieldName);
}
