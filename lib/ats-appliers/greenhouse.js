...
async function greenhouseFieldHasFiles(page, selector) {
  return page.evaluate((targetSelector) => {
    const node = document.querySelector(targetSelector);
    return Boolean(node && (node.files || []).length > 0);
  }, selector);
}
...
async function collectGreenhouseMissingFields(page, applicant, questions = []) {
  ...
  if (!await greenhouseFieldHasFiles(page, '#resume, input[type="file"][id*="resume"]')) missing.push('Resume/CV*');
