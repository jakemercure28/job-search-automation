...
async function collectGreenhouseMissingFields(page, applicant, questions = []) {
  ...
  if (applicant.school) {
    ...
    if (!schoolTarget || greenhouseComboboxSnapshotIsEmpty(schoolTarget)) {
...
async function applyGreenhouse(job, applicant, dryRun = false, draftedAnswers = {}) {
  ...
  try {
    ...
    for (const question of questions) {
      for (const field of (question.fields || [])) {
        const answer = resolveGreenhouseQuestionAnswer(question.label, applicant, draftedAnswers, field.name);
    ...
    if (validation.missing.length) {
      ...
      return {
        ...
        details: { schoolSelection },
        error: `Required fields still empty before submit: ${validation.missing.join(', ')}. Check screenshot: ${incompleteScreenshot}`,
      };
    }

    const preScreenshot = await saveScreenshot(page, job, dryRun ? 'presubmit-dry-run' : 'presubmit');
    if (dryRun) {
      log.info('Greenhouse dry-run validation passed', {
        company: job.company,
        title: job.title,
