...
async function fillGreenhouseEducation(page, applicant) {
  ...
  if (schoolInput) {
    ...
    await page.evaluate((school) => {
      ...
      for (const item of items) {
        if (item.innerText.toLowerCase().includes(String(school).toLowerCase())) {
          item.click();
          return true;
        }
      }
      return false;
    }, applicant.school).catch(() => false);
  ...
  if (degreeSelect) {
    ...
    const match = opts.find((option) => option.toLowerCase().includes('bachelor'));
    if (match) await degreeSelect.select(match).catch(() => {});
  }
