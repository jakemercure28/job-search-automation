...
async function applyGreenhouse(job, applicant, options = {}) {
  ...
  try {
    ...
    // Only do this for non-Greenhouse-hosted URLs — standard job-boards.greenhouse.io
    // URLs with a page issue are a different problem and shouldn't try an Apply click.
    if (pageIssue && overrideApplyUrl && !overrideApplyUrl.includes('greenhouse.io')) {
      ...
      // Dismiss common cookie consent banners before interacting with the page
      try {
        const dismissed = await page.evaluate(() => {
          const selectors = [
            ...
            // Generic accept/allow buttons in consent banners
