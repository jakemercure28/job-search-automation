...
async function chooseGreenhouseComboboxOption(page, fieldName, input, answer) {
  ...
  const picked = await page.evaluate((targetFieldName, desiredAnswer) => {
    ...
    const listboxIds = [target.getAttribute('aria-controls'), target.getAttribute('aria-owns')].filter(Boolean);
    for (const id of listboxIds) {
      collectOptions(document.getElementById(id), optionNodes);
    }

    const group = target.closest('[class*="field"], [class*="form-group"], [role="group"], [class*="select"]');
    collectOptions(group, optionNodes);
    collectOptions(document, optionNodes);
  ...
  return page.evaluate((targetFieldName, desiredAnswer) => {
    function normalize(value) {
      return String(value || '').replace(/\s+/g, ' ').trim().toLowerCase();
    }
    ...
    const renderedValues = [
      ...
      group?.querySelector('[class*="placeholder"]')?.innerText,
      group?.innerText,
    ]
      .map(normalize)
      .filter(Boolean);
  ...
  try {
    ...
    for (const question of questions) {
      for (const field of (question.fields || [])) {
        ...
        const stillEmpty = await page.evaluate((fieldName) => {
          const escapedName = window.CSS && typeof window.CSS.escape === 'function'
