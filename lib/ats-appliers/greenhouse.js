...
function answerForGreenhouseQuestion(label) {
 ...
 */
async function applyGreenhouse(job, applicant, dryRun = false) {
  ...
  if (!applicant.email) return { success: false, error: 'APPLICANT_EMAIL not set in .env' };
  if (!applicant.phone) return { success: false, error: 'APPLICANT_PHONE not set in .env' };

  const resumeAbsPath = path.resolve(process.cwd(), applicant.resumePath);
  if (!fs.existsSync(resumeAbsPath)) {
    return { success: false, error: `Resume not found: ${resumeAbsPath}` };
  }
  ...
  try {
    ...
    for (const question of questions) {
      const answer = answerForGreenhouseQuestion(question.label);
