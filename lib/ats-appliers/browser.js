...
async function getIncompleteRequiredFields(page) {
  return page.evaluate(() => {
    ...
    function hasSatisfiedCombobox(node) {
      ...
      if (!className.includes('requiredInput')) return false;
      const group = node.closest('[class*="field"], [class*="form-group"], [role="group"]');
      if (!group) return false;
      const combobox = group.querySelector('input[role="combobox"], input.select__input');
      const renderedValue = group.querySelector('[class*="single-value"]');
      return Boolean(
        (combobox && (combobox.value || '').trim())
