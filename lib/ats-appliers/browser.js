...
async function launchBrowser({ headless = true } = {}) {
  ...
  browser.on('disconnected', () => {
    log.warn('Browser disconnected', { headless });
  });
  return browser;
}
...
async function newPage(browser) {
  ...
  page.on('error', (err) => log.error('Page crashed', { error: err.message }));
  page.on('pageerror', (err) => log.warn('Page JS error', { error: err.message }));
  return page;
}
...
function splitExpectedMissingFields(missingFields = [], expectedFields = []) {
  const expected = (expectedFields || [])
    .map((field) => {
      if (!field) return null;
      if (typeof field === 'string') return normalizeFieldLabel(field);
      return normalizeFieldLabel(field.label || field.name || '');
    })
    .filter(Boolean);
  ...
  for (const field of missingFields || []) {
    ...
    if (matched) expectedMissing.push(field);
    else blocking.push(field);
  }

  return { blocking, expectedMissing };
}

async function captureAssistPageMeta(page) {
  return page.evaluate(() => {
    const text = String(document.body?.innerText || '').replace(/\s+/g, ' ').trim();
    return {
      pageTitle: document.title || null,
      pageUrl: window.location.href || null,
      textSnippet: text ? (text.length <= 220 ? text : `${text.slice(0, 217)}...`) : null,
    };
  });
}
