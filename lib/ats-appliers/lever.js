...
const { waitForApplicationConfirmation, fetchGreenhouseCode } = require('../gmail-code');
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      if (codeBoxes.length === 8) {
        ...
        if (!code) { log.warn('Security code not found in Gmail within 45s'); return null; }
        ...
        log.info('Security code submitted');
      ...
      if (e.message?.includes('detached') || e.message?.includes('closed') || e.message?.includes('disconnected')) {
        log.info('Browser disconnected, stopping code watcher');
        return null;
      }
      log.warn('Code watcher poll error', { error: e.message });
    }
  }
  log.warn('Code watcher timed out', { maxWaitMs });
  return null;
}
...
async function applyLever(job, applicant, options = {}) {
  ...
  try {
    ...
    // Upload resume — Lever has a file drop zone; wait for it to be present and interactable
    await page.waitForSelector('input[type="file"]', { timeout: 10000 }).catch(() => null);
