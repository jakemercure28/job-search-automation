 ...
 */
async function applyLever(job, applicant, dryRun = false, draftedAnswers = {}, draftedQuestions = []) {
  ...
  try {
    ...
    for (const field of draftedQuestions) {
      ...
      await fillLeverDraftedField(page, field, answer);
    }
    ...
    if (dryRun) {
      log.info('Lever dry-run validation passed', {
        company: job.company,
        title: job.title,
