 ...
 */
async function applyAshby(job, applicant, options = {}) {
  const {
    mode = 'submit',
    answers = {},
    questions = [],
  ...
  try {
    ...
    if (await fillAshbyLabeledField(page, ['first name'], applicant.firstName)) filledFields.push('First name');
    if (await fillAshbyLabeledField(page, ['last name', 'surname'], applicant.lastName)) filledFields.push('Last name');
    ...
    if (applicant.linkedin) {
      if (await fillAshbyLabeledField(page, ['linkedin'], applicant.linkedin)) filledFields.push('LinkedIn');
    }
    if (applicant.github) {
      if (await fillAshbyLabeledField(page, ['github', 'github profile', 'github url'], applicant.github)) filledFields.push('Github Profile');
    }
    ...
    for (const question of questions) {
      ...
      else filledFields.push(question.label || question.name);
    }
