  ...
  for (const job of eligible) {
    ...
    try {
      if (platform === 'greenhouse') {
        result = await applyGreenhouse(job, jobApplicant, dryRun);
      } else if (platform === 'lever') {
        result = await applyLever(job, jobApplicant, dryRun);
      } else {
        result = await applyAshby(job, jobApplicant, dryRun);
      }
    } catch (e) {
...
async function applyOne(db, config, jobId, dryRun = false) {
  ...
  if (job.apply_complexity !== 'simple') return { success: false, error: 'not a simple application' };
  if (job.auto_apply_status != null) return { success: false, error: 'already attempted' };

  const { applicant, blocklist, platformBlocklist } = config;
  const platform = detectPlatform(job);
  ...
  if (isBlocked(job, blocklist)) {
    return { success: false, error: 'company on blocklist' };
  }
  if (isBlockedPlatform(platform, platformBlocklist)) {
    return { success: false, error: `platform ${platform} is on blocklist` };
  }
...
module.exports = { run, applyOne, countSuccessfulApplicationsToday, isBlockedPlatform };
