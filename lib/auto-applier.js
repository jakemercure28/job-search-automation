...
async function planAutoApply(db, config, options = {}) {
  ...
  if (jobId) {
    ...
    jobs = db.prepare(`
      SELECT * FROM jobs
      WHERE COALESCE(auto_apply_status, '') = 'failed'
  ...
  for (const baseJob of sortJobs(jobs, scoreOrder)) {
    ...
    if (!skipReason && job.apply_complexity === 'complex') skipReason = 'complex-form';
...
function updateJobAutoApplyState(db, jobId, attemptedAt, status, error) {
  db.prepare(`
    UPDATE jobs
    SET auto_applied_at   = ?,
        auto_apply_status = ?,
    ...
    WHERE id = ?
  `).run(attemptedAt, status, error || null, status, status, attemptedAt, status, jobId);
}
...
async function applyOne(db, config, jobId, dryRun = false, options = {}) {
  ...
  if (job.apply_complexity === 'complex') return { success: false, error: 'job is marked complex' };
  ...
  if (!dryRun) {
    updateJobAutoApplyState(db, job.id, attemptedAt, status, result.error || null);
    if (result.success) {
