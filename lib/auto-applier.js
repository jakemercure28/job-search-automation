...
function buildApplicantForJob(job, applicant) {
  ...
  return { ...applicantDefaults, ...(applicant || {}), resumePath };
}
...
function updateValidationFailureStreak(state, receipt, threshold = 0) {
  const next = {
    platform: null,
    count: 0,
    shouldHalt: false,
  };
  if (!Number.isInteger(threshold) || threshold <= 0) return next;

  const failureClass = receipt?.failure_class || null;
  const platform = receipt?.platform || null;
  if (failureClass !== 'validation' || !platform) return next;
  ...
  next.shouldHalt = next.count >= threshold;
  return next;
}
...
async function planAutoApply(db, config, options = {}) {
  ...
  for (const baseJob of sortJobs(jobs, scoreOrder)) {
    ...
    const prep = safeGetApplicationPrep(db, job.id);
    let skipReason = null;
    ...
    if (!skipReason && requireReadyPrep && prep?.status !== 'ready') skipReason = 'prep-not-ready';
    const row = {
      jobId: job.id,
      company: job.company,
      title: job.title,
...
async function run(db, config, dryRun = false, options = {}) {
  ...
  const filters = { scoreOrder, minScore, maxScore, platforms, limit, retryFailed, requireReadyPrep, stopOnValidationStreak };
  ...
  let processed = 0;
  let validationStreak = { platform: null, count: 0, shouldHalt: false };
  for (const candidate of candidates) {
    ...
    validationStreak = updateValidationFailureStreak(validationStreak, outcome.receipt || outcome, stopOnValidationStreak);
    if (validationStreak.shouldHalt) {
      log.warn('Stopping auto-apply run after repeated validation failures', {
        platform: validationStreak.platform,
        count: validationStreak.count,
        runId,
      });
      break;
    }
