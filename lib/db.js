...
function canonicalizeAlternateJob(db, alternate, resolution) {
  ...
  if (resolution.status !== 'primary' || !resolution.job?.id) {
    ...
    if (resolution.status === 'unsupported') {
      const scored = alternate.score != null && alternate.score >= 7;
      if (scored) {
        db.prepare("UPDATE jobs SET status = 'needs-manual-review', updated_at = datetime('now') WHERE id = ?").run(alternate.id);
        logEvent(db, alternate.id, 'ats_resolution', alternate.platform, 'needs-manual-review');
      } else {
