...
function getGlobalStats(db) {
  return {
    ...
    notApplied: db.prepare("SELECT COUNT(*) as n FROM jobs WHERE status NOT IN ('applied','responded','archived')").get().n,
    applied: db.prepare("SELECT COUNT(*) as n FROM jobs WHERE applied_at IS NOT NULL").get().n,
